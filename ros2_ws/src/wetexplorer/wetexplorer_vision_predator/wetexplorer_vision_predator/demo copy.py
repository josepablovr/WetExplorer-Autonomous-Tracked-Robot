...
from std_srvs.srv import Trigger  # Using the standard Trigger service


class AlignmentService(Node):
    def __init__(self):
        super().__init__('alignment_service')
        # Create the service
        self.srv = self.create_service(Trigger, '/alignment', self.handle_alignment_request)
        self.get_logger().info("Alignment Service ready and waiting for requests...")

    def handle_alignment_request(self, request, response):
        # Execute the Registration function
        Registration()
        # Send a response
        response.success = True
        response.message = "Registration function executed successfully."
        return response
...
class ThreeDMatchDemo(Dataset):
    ...
    def __getitem__(self,item): 
        ...
        print("SOURCE")
        print(f"Before SRC: {type(src_pcd)}")
        print(f"Before SRC: {len(src_pcd[0])}")
        print(f"Before SRC: {len(src_pcd)}")
        print("TARGET")
        print(f"Before TRG: {type(tgt_pcd)}")
        print(f"Before SRC: {len(tgt_pcd[0])}")
        print(f"Before SRC: {len(tgt_pcd)}")
...
def Registration():
    ...
    config = os.path.join(cwd, 'configs/test/indoor.yaml')  
    ...
    # do pose estimation
    estimate(config, demo_loader)


def main(args=None):
    rclpy.init(args=args)
    node = AlignmentService()
    try:
        rclpy.spin(node)
