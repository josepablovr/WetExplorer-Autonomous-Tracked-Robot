...
class MaskedImagePublisher(Node):
    def __init__(self):
        ...
        # Subscribers
        self.create_subscription(DetectionArray, '/yolo/detections', self.detection_callback, qos_profile=best_effort_qos)
        ...
        self.create_subscription(Image, '/camera/depth/raw', self.depth_callback, qos_profile=best_effort_qos)
        self.create_subscription(Image, '/camera/image/raw', self.color_callback, qos_profile=best_effort_qos)
    ...
    def camera_info_callback(self, msg):
        ...
        self.get_logger().info('Unsubscribed from /camera/camera_info.')
    ...
    def detection_callback(self, msg: DetectionArray):
        ...
        if self.depth_image is None or self.color_image is None or self.camera_info is None:
            ...
            if self.depth_image is None:
                self.get_logger().info('Depth is missing...')
            elif self.color_image is None:
                self.get_logger().info('Color is missing...')
            else:
                self.get_logger().info('Info is missing...')
            return
    ...
    def process_detection(self, detection: Detection):
        ...
        if detection.mask.data:
            ...
            if (detection.class_name == "ring" and detection.score >= 0.9 and detection.bbox.size.x * detection.bbox.size.y > 500):
    ...
    def generate_pointcloud(self):
        ...
        for v in range(h):
            for u in range(w):
                z = self.masked_depth[v, u]  * self.scale
                if z > 0:
                    x = (u - cx) * z / fx
                    y = (v - cy) * z / fy
                    points.append([x, y, z])
    ...
    def save_pointcloud(self, points, index):
        ...
        directory = '/ros2_ws/src/wetexplorer/wetexplorer_vision_predator/train/V3'
