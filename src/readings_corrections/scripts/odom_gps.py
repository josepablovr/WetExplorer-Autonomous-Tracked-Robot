...
class ImuToBaseLink:
    ...
    def geodetic_to_enu(self, lat, lon, alt, ref_lat, ref_lon, ref_alt):
        ...
        try:
            ...
            if self.init_pos is None:
                self.ref_lat = self.latitude
                self.ref_lon = self.longitude
                self.ref_alt = self.altitude
                self.init_pos = True
                return
            ...
            world_quat = tf.transformations.quaternion_from_matrix(
                np.vstack((np.hstack((R_gps_world, [[0], [0], [0]])), [0, 0, 0, 1])))
            ...
            # Set position
            odom_msg.pose.pose.position.x = P_base_world[0]
            odom_msg.pose.pose.position.y = P_base_world[1]
            odom_msg.pose.pose.position.z = P_base_world[2]

            # Set orientation
            odom_msg.pose.pose.orientation.x = world_quat[0]
            odom_msg.pose.pose.orientation.y = world_quat[1]
            odom_msg.pose.pose.orientation.z = world_quat[2]
            odom_msg.pose.pose.orientation.w = world_quat[3]
    ...
    def run(self):
        self.odom_pub = rospy.Publisher('/odom_with_com', Odometry, queue_size=10)
        rospy.spin()
